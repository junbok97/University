...
void read_key_send(int s, char* buf1, char* buf2)
{
	...
	if (fgets(buf1, BUF_LEN, stdin) > 0) {
		...
		if (strncmp(buf1, CHAT_CMD_FILESEND, strlen(CHAT_CMD_FILESEND)) == 0) {
			file_send(s, buf1);
			return;
		}
...
void file_send(int s, char* buf) {
	...
	// buf = /filesend user2 filename

	while (1) {
		memset(buf1, 0, BUF_LEN);
		n = recv(s, buf1, BUF_LEN, 0);
		if (n > 0) {
			break;
		}
	...
	if (strcmp(buf1, CHAT_CMD_FILE_YES) != 0)
		return;
	...
	// user2 에게 filename, filesize 전송
	sprintf(buf1, "%s %d", filename, filesize);
	if (send(s, buf1, BUF_LEN, 0) <= 0) {
		printf("send error\n");
		return;
	}
	...
	memset(buf1, 0, BUF_LEN);
	sprintf(buf1, "%s", CHAT_CMD_FILE_END);
	send(s, buf1, BUF_LEN, 0);
