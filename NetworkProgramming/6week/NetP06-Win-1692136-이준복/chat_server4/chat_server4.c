...
void Wisper(char* buf, int n); // 귓속말 처리 함수
	...
	while (1) {
		...
		for (i = 0; i < num_chat; i++) {
			...
			if (FD_ISSET(client_fds[i], &read_fds)) {
				...
				if (strncmp(buf + n + 1, CHAT_CMD_TO, strlen(CHAT_CMD_TO)) == 0) {
					Wisper(buf, n);
					continue;
				}
				...
				// /sleep 처리
				// [user1] /sleep
				if (strncmp(buf + n + 1, CHAT_CMD_SLEEP, strlen(CHAT_CMD_SLEEP)) == 0) {
					for (j = 0; j < num_chat; j++) {
				...
				// /wakeup 처리
				// [user1] /wakeup
				if (strncmp(buf + n + 1, CHAT_CMD_WAKEUP, strlen(CHAT_CMD_WAKEUP)) == 0) {
					for (j = 0; j < num_chat; j++) {
						if (strncmp(buf1 + 1, userlist[j], strlen(userlist[j])) == 0) {
							usersleep[j] = 0;
							break;
