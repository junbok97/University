...
int main(int argc, char* argv[]) {
	...
	while (1) {
		...
		while (1){
			...
			// 아이디는 맞고 비밀번호는 틀릴 때
			else if ((strcmp(id, ID) == 0) && (strcmp(pass, PASSWORD) != 0)) {
		...
		while (1) {
			...
			msg_size = send(client_fd, buf2, msg_size, 0);
			if (msg_size <= 0) {
				printf("send error\n");
				break;
			}
			printf("Sending len=%d : %s", msg_size, buf2);
		}
